  ...
  return new Promise((resolve, reject) => {
    ...
    const createUsersTable = `
      CREATE TABLE IF NOT EXISTS users (
        ...
        email VARCHAR(100) UNIQUE NOT NULL,
        password VARCHAR(255) NOT NULL,
        userType ENUM('jobseeker') NOT NULL DEFAULT 'jobseeker',
        firstName VARCHAR(50) NOT NULL,
        lastName VARCHAR(50) NOT NULL,
        phone VARCHAR(20),
        location VARCHAR(100),
...
// Enhanced validation function
const validateRequiredFields = ({ firstName, lastName, userName, email, password }) => {
  const errors = [];
...
// Helper function to send welcome email asynchronously
const sendWelcomeEmailAsync = async (userData) => {
  try {
    const emailResult = await sendWelcomeEmail(userData, 'jobseeker');
    if (emailResult && emailResult.success) {
      console.log('Welcome email sent successfully to:', userData.email);
    } else {
      console.warn('Failed to send welcome email:', emailResult?.error || 'Unknown error');
    }
  } catch (emailError) {
    console.warn('Email sending error:', emailError.message);
    // Don't fail registration if email fails
...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Send welcome email (non-blocking)
      sendWelcomeEmailAsync(userData);

      // Send success response
      const responseTime = Date.now() - startTime;
      res.status(201).json({
        success: true,
        msg: 'Registration successful! Welcome to ProFetch!',
        token,
        user: {
          id: userId,
          userName: userData.userName,
